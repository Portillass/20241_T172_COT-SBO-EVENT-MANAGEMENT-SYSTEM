...
router.post('/oauth-callback', async (req, res) => {
  try {
    ...
    if (result.success) {
      ...
      req.session.calendarTokens = result.tokens;
...
router.get('/events', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.session.calendarTokens) {
      calendarService.setCredentials(req.session.calendarTokens);
    }
...
// Update event
router.put('/events/:eventId', [authenticateToken, validateEvent], async (req, res) => {
  try {
    if (req.session.calendarTokens) {
      calendarService.setCredentials(req.session.calendarTokens);
    }
